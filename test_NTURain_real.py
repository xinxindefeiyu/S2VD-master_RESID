...
for current_type in rain_types:
    ...
    print('inds_ext_start' + str(inds_ext_start[0]))
    print('inds_ext_end' + str(inds_ext_end[0]))

    derain_data = torch.zeros_like(rain_data)
    print('len is ' + str(len(inds_start)))
    ...
    savemat('testsets/' + current_type+'.mat', {'derain_data':img_as_ubyte(derain_data.squeeze(0).permute([2,3,0,1]).numpy()),
